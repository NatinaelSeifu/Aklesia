...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 እንኳን ወደ የቀሲስ ጥላሁን ጉደታ የንስሐ ልጆች አቅሌስያ መጡ!\nእባክዎ ለመጀመር /register የሚለውን በመጫን አካውንት ይክፈቱ."
    )
...
async def set_commands(app):
    ...
    # Set default (non-admin) commands explicitly
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(register.handle_edit_profile_callback,
        pattern="^(edit_name|edit_phone|edit_email|edit_joined_on|edit_children|married|single|delete_profile|confirm_delete_yes|confirm_delete_no)$"))
    ...
    app.add_handler(CallbackQueryHandler(book.handle_mybookings_callback,
        pattern=r"^change_[a-f0-9-]+$|^abort_cancel_|^confirm_cancell_[a-f0-9-]+$|^cancel_[a-f0-9-]+$|^confirm_change_[a-f0-9-]+_\d{4}-\d{2}-\d{2}$"))
    ...
    app.add_handler(CallbackQueryHandler(admin.handle_cancel_avail_callback,
        pattern=r"^cancel_avail_\d{4}-\d{2}-\d{2}$|^confirm_cancel_\d{4}-\d{2}-\d{2}$|^avail_cancel_back$|^cancel_avail_menu$"))
    app.add_handler(CommandHandler("question", admin.handle_view_questions))
    app.add_handler(CallbackQueryHandler(admin.handle_admin_question_callback,
        pattern=r"^question_cancel_[a-f0-9-]+|^question_complete_[a-f0-9-]+"))
    ...
    app.add_handler(CallbackQueryHandler(admin.handle_admin_communion_callback,
        pattern=r"^communion_complete_[a-f0-9-]+|^communion_cancel_[a-f0-9-]+"))
